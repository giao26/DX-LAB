...
CREATE TABLE IF NOT EXISTS dx_core.announcements (
  ...
  CONSTRAINT announcements_scope_target_check CHECK (
    (scope = 'company' AND target_group IS NULL)
    OR (scope = 'group' AND NULLIF(BTRIM(target_group), '') IS NOT NULL)
  ),
  CONSTRAINT announcements_title_not_blank_check CHECK (NULLIF(BTRIM(title), '') IS NOT NULL),
  CONSTRAINT announcements_publication_window_check CHECK (expires_at IS NULL OR expires_at > published_at)
);
...
INSERT INTO dx_core.announcements (id, title, body, scope, target_group, published_at)
VALUES
  ('70000000-0000-4000-8000-000000000001', 'Chào mừng đến DX-OS – hệ thống quản lý vận hành số',
  ...
  ('70000000-0000-4000-8000-000000000002', 'Lịch bảo trì hệ thống tháng 10/2026',
  ...
  ('70000000-0000-4000-8000-000000000003', 'Quy trình onboarding nhân viên mới cập nhật',
...
-- Story 3.1: Reporting view, CSAT ratings, and dxlab_reporter role
